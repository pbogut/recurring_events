defmodule RecurringEvents.DailyTest do
  use ExUnit.Case
  doctest RecurringEvents.Daily

  alias RecurringEvents.Daily
  ...
  test "for until ~D[2017-12-29] it should return 2 events" do
    until = ~D[2017-12-29]
    events = Daily.unfold(@date, @valid_rrule |> Map.put(:until, until))
  ...
  test "with no count, until and interval it should stream forever" do
    events = Daily.unfold(@date, @valid_rrule)
  ...
  test "for count 5 it should return 5 events" do
    events = Daily.unfold(@date, @valid_rrule |> Map.put(:count, 5))
