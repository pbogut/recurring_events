defmodule RecurringEvents.Freq.MonthlyTest do
  use ExUnit.Case
  doctest RecurringEvents.Freq.Monthly

  alias RecurringEvents.Freq.Monthly
  ...
  test "for until ~D[2017-02-01] it should return 2 events" do
    until = ~D[2017-02-01]
    {:ok, events} = Monthly.unfold(@date, @valid_rrule |> Map.put(:until, until))
  ...
  test "with no count, until and interval it should stream events forever" do
    {:ok, events} = Monthly.unfold(@date, @valid_rrule)
  ...
  test "for count 5 it should return 5 events" do
    {:ok, events} = Monthly.unfold(@date, @valid_rrule |> Map.put(:count, 5))
    assert 5 == Enum.count(events)
  end

  test "for interval 5 it should return events every 5 months" do
    {:ok, events} = Monthly.unfold(@date, @valid_rrule |> Map.put(:interval, 5))
